jatos.onLoad(async () => {
  // Restore fullscreen if it was active in previous component
  if (jatos.studySessionData.isFullscreen && !document.fullscreenElement) {
    try {
  ...
  let n_attention_failed_so_far = 0;
  let n_total_attention_checks = 0;
  let attention_correct = true;

    if (jatos.studySessionData && jatos.studySessionData.n_attention_failed_so_far !== undefined) {
        n_attention_failed_so_far = jatos.studySessionData.n_attention_failed_so_far;
    }

    if (jatos.studySessionData && jatos.studySessionData.n_total_attention_checks !== undefined) {
      n_total_attention_checks = jatos.studySessionData.n_total_attention_checks;
  }

  console.log("n_attention_failed_so_far", n_attention_failed_so_far)
  ...
  const results = {
    ...
    attention_correct: null,
    n_attention_failed_so_far: 0,
    n_total_attention_checks: 0,
  };
  ...
  function submitFinalAnswers() {
    if (review_page_disabled || userScrolledAllTheWayDown) {
      ...
      results.attention_correct = attention_correct;
      results.n_attention_failed_so_far = n_attention_failed_so_far;
      results.n_total_attention_checks = n_total_attention_checks;
      jatos.studySessionData.n_attention_failed_so_far = n_attention_failed_so_far;
      jatos.studySessionData.n_total_attention_checks = n_total_attention_checks;
      ...
      if (userScore >= risk_threshold || flag_risk_screen) {
        ...
        console.log("n_attention_failed_so_far", n_attention_failed_so_far)
        if (n_attention_failed_so_far >= 2) {
          jatos.studySessionData.isFullscreen = !!document.fullscreenElement;
